...
def main1(team, ground, oversGone, runs, wickets):
    ...
    score=predict_score(team,teamAverage,groundAverage,runs,wickets)

    predicted_run = score[0][0]
    if (predicted_run + .5) < math.ceil(predicted_run):
        predicted_run = math.floor(predicted_run)
    elif (predicted_run + .5) > math.ceil(predicted_run):
        predicted_run = math.ceil(predicted_run)
        # print("Dhuksi")

    predicted_wicket = score[0][1]
    if (predicted_wicket + .5) < math.ceil(predicted_wicket):
        predicted_wicket = math.floor(predicted_wicket)
    else:
        predicted_wicket = math.ceil(predicted_wicket)
    #print(predicted_run, " ", predicted_wicket)

    return  predicted_run, predicted_wicket
